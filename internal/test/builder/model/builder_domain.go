...
func NewDomain(gormModel gorm.Model) Domain {
	...
	case 1:
		title = pointy.String(builder_helper.GenRandomString(letters, int(builder_helper.GenRandNum(5, 20))))
	case 2:
		title = nil
	default:
	...
	switch builder_helper.GenRandNum(1, 2) % 2 {
	case 1:
		description = pointy.String(builder_helper.GenRandomString(letters, int(builder_helper.GenRandNum(10, 300))))
	case 2:
		description = nil
	default:
		panic("something went wrong for random title")
	...
	return &domain{
		Domain: model.Domain{
			...
			DomainUuid:            uuid.New(),
			DomainName:            pointy.String(builder_helper.GenRandomDomainName(2)),
